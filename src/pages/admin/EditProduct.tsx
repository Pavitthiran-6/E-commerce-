...
export default function EditProduct() {
  ...
  const [editNewImages, setEditNewImages] = useState<ImageFile[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // 1. Fetch categories
        const catRes = await axiosInstance.get('/api/categories/tree');
        const tree: CategoryNode[] = catRes.data?.data || [];
        ...
        if (product) {
          ...
          setDiscount(String(product.discountPercentage || product.discount || ''));
          setStockQuantity(String(product.stockQuantity || ''));
          setDescription(product.description || '');
          setInStock(product.inStock ?? true);
          setIsNew(product.isNew ?? false);
          setIsBestseller(product.isBestseller ?? false);
          setIsApparelHighlights(product.isApparelHighlights ?? false);
          setIsTechHome(product.isTechHome ?? false);

          const images: string[] = product.images || (product.image ? [product.image] : []);
          setExistingImages(images);
          setExistingCover(images[0] || '');

          // Resolve loaded categories mapping
          let parentId = '';
          let subId = '';
          const slug = product.category || '';
          for (const parent of tree) {
            if (parent.slug === slug) { parentId = String(parent.id); break; }
            for (const sub of parent.children) {
              if (sub.slug === slug) { parentId = String(parent.id); subId = String(sub.id); break; }
            }
            if (parentId) break;
          }
          ...
          setSpecs(rawSpecs.map((s: any, i: number) => ({ key: s.key || '', value: s.value || '', displayOrder: s.displayOrder ?? i })));
          setMaterialsTitle(product.materialsTitle || 'MATERIALS');
          setMaterialsContent(product.materialsContent || '');
          setShippingTitle(product.shippingTitle || 'SHIPPING & RETURNS');
          setShippingContent(product.shippingContent || '');
          setCareTitle(product.careTitle || 'PRODUCT CARE');
          setCareContent(product.careContent || '');
          setSustainabilityTitle(product.sustainabilityTitle || 'SUSTAINABILITY');
          setSustainabilityContent(product.sustainabilityContent || '');
  ...
  useEffect(() => {
    return () => {
      ...
      editNewImages.forEach(img => URL.revokeObjectURL(img.preview));
    };
  }, [coverPreview, editNewImages]);
  ...
  const handleCoverChange = (files: FileList | null) => {
    ...
    // When a new cover file is selected, remove existingCover from visual display
    setExistingCover('');
  };
  ...
  const removeExistingImage = (url: string) => {
    const next = existingImages.filter(u => u !== url);
    setExistingImages(next);
    setExistingCover(existingCover === url ? (next[0] || '') : existingCover);
  };
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-gray-400 font-medium">Loading product details...</div>;
  }

  // Resolve cover preview to show in the live preview & cover box
  const resolvedCoverPreview = coverPreview || existingCover || '';
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-500">Modify properties or assets for this catalog item</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              {resolvedCoverPreview ? (
                <div className="relative rounded-xl overflow-hidden border border-gray-200 aspect-[4/3] max-w-md">
                  <img src={resolvedCoverPreview} alt="Cover preview" className="w-full h-full object-cover" />
            ...
            {existingImages.filter(url => url !== resolvedCoverPreview).length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-500">Current Secondary Images</p>
            ...
            {/* Add new images */}
            <div className="space-y-2">
              <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500">Upload Additional Images</label>
              <ImageUploadArea images={editNewImages} onChange={setEditNewImages} />
            </div>
